...
variablen () {
    invoke_two=$(date '+%s.%N') # überall im Skript durch date +%s.%N
    (curl -X POST -H "Content-Type:application/json" -H "Authorization: bearer $(gcloud auth print-identity-token)" https://europe-west1-intrepid-tape-368615.cloudfunctions.net/matrixmult_small_two_$i -d '{"A": [[1,5,2],[3,4,5],[2,3,4]], "B": [[6,5,2],[3,9,5],[7,3,7]]}')> result.txt
    return_two=$(date '+%s.%N')
        
    id_two=$(sed -n 1p result.txt) # hier wird die execution-id, aus der Funktionsrückgabe gefiltert
    start_two=$(sed -n 2p result.txt)
    stop_two=$(sed -n 3p result.txt)
    realtype_two=$(sed -n 4p result.txt)
    ...
    sed -i "1 i\SET @invoke_$type_two := $invoke_two;" mysqldata.txt
    sed -i "1 i\SET @return_$type_two := $return_two;" mysqldata.txt   
    sed -i "1 i\SET @start_$type_two := $start_two;" mysqldata.txt  
    sed -i "1 i\SET @stop_$type_two := $stop_two;" mysqldata.txt
    sed -i "1 i\SET @id_$type_two := '$id_two';" mysqldata.txt
    # Boolean, der angibt ob es wirklich ein Cold start wat
    sed -i "1 i\SET @cold_$type_two := '$realtype_two';" mysqldata.txt

    # die Logs brauchen ein paar Sekunden, um für den Funktionsaufruf erstellt zu werden
    sleep 20
    gcloud functions logs read matrixmult_small_two_$i --execution-id $id_two > log_google.txt
    if [ -s log_google.txt ]
    then
        execution_two=$(grep -o "took\(.*\)ms" log_google.txt)  
        # Die Log - Daten werden im richtigen Fomat in die Text Datei geschrieben
        sed -i "1 i\SET @log_executiontime_$type_two := $execution_two;" mysqldata.txt
    fi 
}
