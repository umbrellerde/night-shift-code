...
variablen () {
    ...
    (curl -X POST -H "Content-Type:application/json" -H "Authorization: bearer $(gcloud auth print-identity-token)" https://europe-west1-intrepid-tape-368615.cloudfunctions.net/matrixmult_big_thirteen_$i -d '{"A": [[1,5,2],[3,4,5],[2,3,4]], "B": [[6,5,2],[3,9,5],[7,3,7]]}')> result.txt
    ...
    sleep 20
    gcloud functions logs read matrixmult_big_thirteen_$i --execution-id $id_thirteen > log_google.txt
    if [ -s log_google.txt ]
    then
