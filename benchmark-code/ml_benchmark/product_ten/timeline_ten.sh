...
variablen () {
    ...
    (curl -X POST -H "Content-Type:application/json" -H "Authorization: bearer $(gcloud auth print-identity-token)" https://europe-west1-intrepid-tape-368615.cloudfunctions.net/ml_middle_ten_$i -d '{}')> result.txt
    return_ten=$(date '+%s.%N')
    ...
    sleep 20
    gcloud functions logs read ml_middle_ten_$i --execution-id $id_ten > log_google.txt
    if [ -s log_google.txt ]
    then
