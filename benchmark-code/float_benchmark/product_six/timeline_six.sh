...
variablen () {
    ...
    (curl -X POST -H "Content-Type:application/json" -H "Authorization: bearer $(gcloud auth print-identity-token)" https://europe-west1-intrepid-tape-368615.cloudfunctions.net/float_middle_six_$i -d '{"N": 15000}')> result.txt
    return_six=$(date '+%s.%N')
    ...
    sleep 20
    gcloud functions logs read float_middle_six_$i --execution-id $id_six > log_google.txt
    if [ -s log_google.txt ]
    then
