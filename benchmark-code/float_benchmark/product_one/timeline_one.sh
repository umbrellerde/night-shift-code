...
variablen () {
    ...
    (curl -X POST -H "Content-Type:application/json" -H "Authorization: bearer $(gcloud auth print-identity-token)" https://europe-west1-intrepid-tape-368615.cloudfunctions.net/float_small_one_$i -d '{"N": 15000}')> result.txt
    return_one=$(date '+%s.%N')
    ...
    sleep 20
    gcloud functions logs read float_small_one_$i --execution-id $id_one > log_google.txt
    if [ -s log_google.txt ]
    then
