...
variablen () {
    ...
    (curl -X POST -H "Content-Type:application/json" -H "Authorization: bearer $(gcloud auth print-identity-token)" https://europe-west1-intrepid-tape-368615.cloudfunctions.net/float_big_fifteen_$i -d '{"N": 15000}')> result.txt
    ...
    sleep 20
    gcloud functions logs read float_big_fifteen_$i --execution-id $id_fifteen > log_google.txt
    if [ -s log_google.txt ]
    then
